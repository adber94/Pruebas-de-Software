...
import javax.swing.table.TableColumn;
	public class Admin extends JFrame implements ActionListener, MouseListener, KeyListener 
	{
	    ...
	    ImageIcon icono = new ImageIcon("admini.png"); //icono de usuario
        ...
        private final JLabel lblRcf = new JLabel("RCF");
	    ...
	    public Admin() {
	        //posiciones etiquetas
	        	...
	        	//lblrol.setBounds(20,270,180,19);
	        	
	        	//rol.setBounds(150,270,180,19);
	        	...
	        	cmdagregar.setBounds(210,431,150,59);
	        	cmdregresar.setBounds(967,431,170,59);
	        	cmdmostrar.setBounds(384,431,179,59);
	        	cmdModificar.setBounds(573,431,191,59);
	        	cmdlimpiar.setBounds(777,431,180,59);
	        	cmdconfirmar.setBounds(650,400,110,23);
	            ...
	            panel.setBounds(210, 170, 943, 226);
	         ...
	         //   JTable tabla = null;
	            ...
	            lblNewLabel.setBounds(254, 152, 60, 14);
	            
	            getContentPane().add(lblNewLabel);
	            lblDomicilio.setFont(new Font("Tahoma", Font.PLAIN, 13));
	            lblDomicilio.setBounds(434, 152, 60, 14);
	            
	            getContentPane().add(lblDomicilio);
	            lblCp.setFont(new Font("Tahoma", Font.PLAIN, 13));
	            lblCp.setBounds(640, 152, 39, 14);
	            
	            getContentPane().add(lblCp);
	            lblRcf.setFont(new Font("Tahoma", Font.PLAIN, 13));
	            lblRcf.setBounds(704, 152, 60, 14);
	            
	            getContentPane().add(lblRcf);
	            lblNombreUsuario.setFont(new Font("Tahoma", Font.PLAIN, 13));
	            lblNombreUsuario.setBounds(847, 152, 110, 14);
	            
	            getContentPane().add(lblNombreUsuario);
	            lblContrasea.setFont(new Font("Tahoma", Font.PLAIN, 13));
	            lblContrasea.setBounds(1020, 153, 117, 14);
	    	  ...
	    	  ConexionBD con=new ConexionBD(); 
				try {
					 Statement st =con.cone();
			  		 String consultaSQL ="INSERT INTO administrador (nombre,domicilio,cp,rfc,usuario_login,contra) VALUES('"+txtnombre.getText()+"' , '"+txtcalle.getText()+"' , '"+txtcp.getText()+"' , '"+txtrfc.getText()+"' , '"+txtnombre_user.getText()+"' , '"+txtcontra.getText()+"')";
	    ...
	    public void mostrar(){
	       int cp=0;
	    	limpiar();
				...
				try {
					 Statement st =con.cone();
			  		 String consultaSQL ="SELECT *FROM administrador";
	    ...
	    
	    {
			...
			try {
				int cp=0;
				 Statement st =con.cone();
		  		 String consultaSQL ="SELECT *FROM administrador WHERE usuario_login='"+txtbuscar.getText()+"';";
		  		 resultado= st.executeQuery(consultaSQL);
		  		 	while(resultado.next()){
					String nombre = resultado.getString("nombre");
					String domicilio = resultado.getString("domicilio");
			 ...
			 ConexionBD con=new ConexionBD(); 
				try {
					 Statement st =con.cone();
			  		 ...
			  		 String consultaSQL ="DELETE FROM administrador WHERE usuario_login='"+txteliminar.getText()+"';";
			  		 st.executeUpdate(consultaSQL);
			  		 st.close();
...
public void modificar()
		 ...
		 {
			 String idModificar = JOptionPane.showInputDialog("Ingresa el nombre de usuario del administrador a modificar: ");
			 if(idModificar != null)
			 {
				...
				try {
					int cp=0;
					 Statement st =con.cone();
			  		 ...
			  		 String consultaSQL ="SELECT *FROM administrador WHERE usuario_login='"+idModificar+"';";
			  		 resultado= st.executeQuery(consultaSQL);
			  		 	while(resultado.next()){
		 ...
		 public void confirmar()
		    {
				...
				txtnombre_user.setEditable(true);
		    	  ConexionBD con=new ConexionBD(); 
					try {
						 Statement st =con.cone();
						 String consultaSQL=  "UPDATE administrador SET nombre='"+txtnombre.getText()+"' WHERE nombre='"+nombre+"';";
						 String consultaSQL2= "UPDATE administrador SET domicilio='"+txtcalle.getText()+"' WHERE domicilio='"+domicilio+"';";
						 String consultaSQL3= "UPDATE administrador SET cp='"+txtcp.getText()+"' WHERE cp='"+CP+"';";
						 String consultaSQL4= "UPDATE administrador SET rfc='"+txtrfc.getText()+"' WHERE rfc='"+RFC+"';";
						 String consultaSQL5= "UPDATE administrador SET contra='"+txtcontra.getText()+"' WHERE contra='"+contra+"';";
